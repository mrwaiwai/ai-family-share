...
const pickExercises = () => [...exercises].sort(() => Math.random() - 0.5).slice(0, 5);
...
const OpenResponsePracticePage = () => {
  ...
  const [selectedExercises, setSelectedExercises] = useState<Exercise[]>(pickExercises);
          ...
          {!submitted ? (
            ...
            <div className="mt-6 space-y-5">
              <div className={`rounded-[22px] border p-5 ${feedback.tone === "rose" ? "border-[#f5b8c7] bg-[#fff0f4]" : feedback.tone === "mint" ? "border-[#b8e5ca] bg-[#effcf4]" : feedback.tone === "sky" ? "border-[#b7e4ee] bg-[#effbfe]" : "border-[#f0d979] bg-[#fff9df]"}`}>
                <div className="flex gap-3"><Lightbulb className="mt-0.5 h-5 w-5 shrink-0 text-secondary" /><div><p className="font-display text-lg font-extrabold text-ink">{feedback.title}</p><p className="mt-1 text-sm font-semibold leading-relaxed text-ink/75">{feedback.text}</p></div></div>
              </div>
              <div className="rounded-[22px] border border-soft bg-soft-sky p-5">
                <p className="font-display text-lg font-extrabold text-ink">{isEn ? "Ideas you could also try" : "你也可以這樣說"}</p>
                <div className="mt-3 space-y-2">{currentExercise.examples.map((example) => <p key={example.zh} className="flex gap-2 text-sm font-semibold leading-relaxed text-ink/80"><ChevronRight className="mt-1 h-4 w-4 shrink-0 text-secondary" />{example[language]}</p>)}</div>
              </div>
              <div className="grid gap-3 rounded-[22px] border border-soft bg-white p-5 sm:grid-cols-4">{[
